...
中断：[THEATER_END]理由`;
    ...
    async _confirmExit(who) {
        ...
        if (who === 'ai') {
            const msg = `${s?.charName}（皮下：${fn}）想结束。\n退出？`;
            const ok = window.zpConfirm ? await window.zpConfirm('次元剧场',msg,'退出','继续') : confirm(msg);
            if(!ok){this._openTheaterUI();return;}
            this._endTheater('end', floorCount);
        } else {
            // 用户退出：选择暂停还是结束
            const choice = floorCount > 0 ?
                (window.zpConfirm ? await window.zpConfirm('次元剧场', '暂停演绎：保存进度，下次继续\n结束剧本：归档封存', '暂停', '结束') : true) : true;
            
            if (choice === null || choice === undefined) return; // 取消
            this._endTheater(choice ? 'pause' : 'end', floorCount);
        }
    }
    
    _endTheater(mode, floorCount) {
        // 清理所有OOC面板
        ...
        if (this._session) {
            this._session.status = mode; // 'pause' or 'end'
            this._saveCurrentSession();
        }
        this._active = false;
